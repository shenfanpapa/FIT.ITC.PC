(()=>{
  ...
  let pass='';
  const history=[]; // Current-page-only memory: 10 user/assistant turns.
  ...
  const sendMessage=async()=>{const field=input.querySelector('input'),send=input.querySelector('.whaleSend'),reply=input.querySelector('.whalePetReply'),message=field.value.trim();if(!message)return;field.value='';field.disabled=true;send.disabled=true;send.textContent='考えています…';reply.textContent='考えています…';try{const response=await fetch('/api/pet/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message,password:pass,history})});const text=await response.text();let data={};try{data=JSON.parse(text);}catch(_error){}if(!response.ok)throw new Error(data.message||`HTTP ${response.status}`);reply.textContent=data.answer||data.message||'返事を受け取れませんでした。';history.push({role:'user',text:message},{role:'assistant',text:reply.textContent});if(history.length>20)history.splice(0,history.length-20);window.whalePetSay?.(reply.textContent,8000);}catch(error){reply.textContent=error.message||'接続できませんでした。もう一度お試しください。';window.whalePetSay?.(reply.textContent,6000);}finally{field.disabled=false;send.disabled=false;send.textContent='送信';}};
