(()=>{
  ...
  password.querySelector('.whaleConfirm').onclick=()=>{const value=password.querySelector('input').value.trim();if(!value)return;pass=value;localStorage.setItem('fit_pet_verified',pass);close(password);input.querySelector('.whalePetReply').textContent='今日は何を確認しますか？';input.hidden=false;input.querySelector('input').focus();};
  password.querySelector('input').addEventListener('keydown',event=>{if(event.key==='Enter')password.querySelector('.whaleConfirm').click();});
  const sendMessage=async()=>{const field=input.querySelector('input'),send=input.querySelector('.whaleSend'),reply=input.querySelector('.whalePetReply'),message=field.value.trim();if(!message)return;send.disabled=true;send.textContent='考え中…';reply.textContent='考えています…';try{const response=await fetch('/api/pet/chat',{method:'POST',headers:{'Content-Type':'application/json','x-pet-password':pass},body:JSON.stringify({message})});const text=await response.text();let data={};try{data=JSON.parse(text);}catch(_error){}if(!response.ok)throw new Error(data.message||`HTTP ${response.status}`);reply.textContent=data.answer||data.message||'返事を受け取れませんでした。';bubble.textContent=reply.textContent;bubble.classList.add('show');}catch(error){reply.textContent=error.message||'接続できませんでした。もう一度お試しください。';}finally{send.disabled=false;send.textContent='送信';}};
