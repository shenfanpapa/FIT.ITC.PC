(()=>{
  ...
  password.querySelector('.whaleConfirm').onclick=()=>{const value=password.querySelector('input').value.trim();if(!value)return;pass=value;password.querySelector('input').value='';close(password);input.querySelector('.whalePetReply').textContent='今日は何を確認しますか？';input.hidden=false;input.querySelector('input').focus();};
