(()=>{
  ...
  const sendMessage=async()=>{const field=input.querySelector('input'),send=input.querySelector('.whaleSend'),reply=input.querySelector('.whalePetReply'),message=field.value.trim();if(!message)return;send.disabled=true;send.textContent='考え中…';reply.textContent='考えています…';try{const response=await fetch('/api/pet/chat',{method:'POST',headers:{'Content-Type':'application/json','x-pet-password':pass},body:JSON.stringify({message})});const data=await response.json();reply.textContent=data.answer||data.message||'返事を受け取れませんでした。';bubble.textContent=reply.textContent;bubble.classList.add('show');if(response.status===401){pass='';localStorage.removeItem('fit_pet_verified');}}catch(_error){reply.textContent='接続できませんでした。もう一度お試しください。';}finally{send.disabled=false;send.textContent='送信';}};
