(()=>{
  ...
  function tick(){if(reduced.matches){put();active.pause();frameId=null;return;}if(!dragging){vy+=.35;y+=vy;const f=floor();if(y<=0)vy=Math.abs(vy)*.35;if(y>=f){y=f;vy*=-.18;if(Math.abs(vy)<.8)vy=0;}if(mode==='walk'){x+=vx;if(x<=0||x>=innerWidth-208){x=Math.max(0,Math.min(innerWidth-208,x));vx*=-1;setFacing();}}}put();frameId=requestAnimationFrame(tick);}
