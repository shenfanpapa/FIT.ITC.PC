(()=>{
  ...
  function tick(){if(reduced.matches){put();active.pause();frameId=null;return;}if(!dragging){vy+=.35;y+=vy;const f=floor();if(y<=0)vy=Math.abs(vy)*.35;if(y>=f){y=f;vy*=-.18;if(Math.abs(vy)<.8)vy=0;}if(mode==='walk'){const remaining=Number.isFinite(walk.duration)?Math.max(0,walk.duration-walk.currentTime):2;const speedScale=remaining<2?Math.max(.06,remaining/2):1;x+=vx*speedScale;if(x<=0||x>=innerWidth-208){x=Math.max(0,Math.min(innerWidth-208,x));vx*=-1;setFacing();}}}put();frameId=requestAnimationFrame(tick);}
