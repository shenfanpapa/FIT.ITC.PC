(()=>{
  ...
  const renderList=card=>{card.innerHTML=`<div class="manual-head"><div><div class="manual-kicker">操作ガイド</div><h2>FIT.ITC.PC 使用手冊</h2><p>知りたい機能を選んでください。</p></div><button class="ux-help-close" aria-label="閉じる">×</button></div><div class="manual-topic-list">${topics.map(topic=>`<button type="button" data-topic="${topic[0]}"><strong>${esc(topic[1])}</strong><span>詳細を見る</span></button>`).join('')}</div>`;};
  ...
  window.whaleGuide=id=>{enhance();const key=String(id||'').toLowerCase();if(key==='all-ok'){const target=targets[key]();target?.classList.add('whale-guide-pulse');setTimeout(()=>target?.classList.remove('whale-guide-pulse'),3400);return;}oldGuide?.(key);};
