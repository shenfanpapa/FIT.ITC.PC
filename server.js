...
const PC_PASS=process.env.PET_CHAT_PASSWORD||'',PC_USE=new Map();app.post('/api/pet/chat',async(q,z)=>{if(!process.env.OPENAI_API_KEY||!PC_PASS)return z.status(503).json({message:'AI未设置。'});if(q.get('x-pet-password')!==PC_PASS)return z.status(401).json({message:'密码错误。'});let m=String(q.body?.message||'').trim(),k=String(q.ip||'x'),u=PC_USE.get(k)||{n:0,t:0,d:new Date().toDateString()};if(u.d!==new Date().toDateString())u={n:0,t:0,d:new Date().toDateString()};if(!m||m.length>600)return z.status(400).json({message:'问题限600字。'});if(Date.now()-u.t<30000)return z.status(429).json({message:'请30秒后再问。'});if(u.n>=12)return z.status(429).json({message:'今日12次已用完。'});u.n++;u.t=Date.now();PC_USE.set(k,u);try{let r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:'Bearer '+process.env.OPENAI_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5.4-mini',instructions:'あなたはFIT.ITC.PCの案内ペット「鯨」です。日本語で、丁寧で少しだけツンとした可愛い口調で答えます。大肥魚ではなく鯨です。サイトの点検、履歴、グループ、レイアウト、テーマ、復元の使い方を短い手順で案内します。業務ガイドでは清掃、機器確認、学生対応、忘れ物、施錠消灯、Teams報告を説明できます。個人名、連絡先、学籍番号などの個人情報は扱いません。分からない内容は職員へ報告するよう案内してください。',input:m,max_output_tokens:400,store:false,reasoning:{effort:'none'}})}),j=await r.json(),a=j.output_text||(j.output||[]).flatMap(x=>x.content||[]).filter(x=>x.type==='output_text').map(x=>x.text).join('');if(!r.ok||!a){console.error('OpenAI pet error',r.status,j?.error);throw 0};z.json({answer:a,remaining:12-u.n})}catch(e){z.status(502).json({message:'AI暂时不可用。'})}});